/**
 * 逻辑闭环校验 API 路由（T155 全量校验）
 *
 * 路由：POST /api/validate/logic
 * 入参：{ scriptId: string; reportType: 'LOGIC' | 'FULL' }
 *
 * 复用本地算法层（prompt 构造 + AI provider + 时间线 / 难度 / 叙诡检测），
 * 不依赖 Supabase Edge Function 部署，与 /api/validate 模式一致，避免 dev 环境 404。
 *
 * 流程：
 *   1. 加载剧本完整数据（characters / acts / scenes / clues / version_snapshots）
 ...
 *
 * 响应：
 *   - 200：{ scriptId, reportType, issues, grouped, tricks, timeline?, difficulty?, reportId, createdAt }
 *   - 400：{ error } 参数缺失或非法
...
import {
  TimelineExtractor,
  type TimelineEvent,
} from '@/lib/validation/timeline/extractor';
import {
  ConflictDetector,
  type ConflictItem,
...
// ===== 数据库行类型（snake_case → camelCase 映射由代码完成） =====
...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      const { data: reportRow, error: reportError } = await persistClient
      ...
      // 写库失败不影响校验结果返回
      console.warn(
        'validation_reports persistence skipped:',
    ...
    return NextResponse.json(
      { error: `逻辑校验失败: ${message}`, scriptId },
      { status: 500 },
    );
  }
}

export type { LogicRequestBody, LogicValidationResponse, ConflictSeverity };
