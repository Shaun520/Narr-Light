// Kimi Provider - 剧本生成与逻辑校验
// 基于 Moonshot/Kimi OpenAI 兼容接口实现，擅长长文本创作与逻辑推理
...
// Mock 模式返回的示例剧本内容
const MOCK_CONTENT = `【Mock 模式 - 未配置 KIMI_API_KEY】
...
/**
 * Kimi Provider 实现
 * 调用 Moonshot/Kimi OpenAI 兼容接口完成文本生成、流式输出、JSON 生成与逻辑校验
 */
export class KimiProvider implements AIProvider {
  ...
  constructor(config?: Partial<ProviderRuntimeConfig>) {
    this.apiKey =
      ...
      "";
    this.baseUrl = normalizeKimiBaseUrl(
  ...
  async generate(options: GenerateOptions): Promise<string> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      try {
        ...
        if (controller.signal.aborted && !options.signal?.aborted) {
          lastError = new Error(`Kimi 请求超时（${this.timeout} 秒），请在 Admin 模型配置中调高文本模型超时时间后重试`);
        }
    ...
    throw lastError ?? new Error("Kimi generate failed after retries");
  }
  ...
  async *generateStream(
    ...
    try {
      ...
      if (!response.body) {
        throw new Error("Kimi stream response body is empty");
      }
      ...
      if (controller.signal.aborted && !options.signal?.aborted) {
        throw new Error(`Kimi 流式生成超时（${this.timeout} 秒），请在 Admin 模型配置中调高文本模型超时时间后重试`);
      }
      throw error;
    } finally {
      clearTimeout(timeoutId);
  ...
  /**
   * 插画生成 - Kimi 为纯文本模型，不支持插画生成
   */
  illustrate(
    prompt: string,
    options?: Record<string, unknown>,
  ): Promise<IllustrationResult> {
    void options; // 接口要求保留参数，Kimi 不支持插画
    return Promise.reject(
      new Error(
        `Kimi 是文本模型，不支持插画生成（收到请求：${prompt.slice(0, 50)}）。请使用 GLM Provider 调用 CogView。`,
      ),
    );
  }
  ...
  private async wrapError(response: Response): Promise<Error> {
    ...
    return new Error(
      `Kimi API error ${response.status}: ${detail || response.statusText}`,
    );
  }
